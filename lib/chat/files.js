...
async function draftMode({ headers, rows, redactedRows, vault, protectedCount, protectedByType, instruction, providerCfg, fileName }) {
  // Prefer overdue customers if a days-past-due column exists; cap for a tidy demo.
  const dpdCol = matchHint(headers, FINANCIAL_HINTS.daysPastDue);
  let indices = rows.map((_, i) => i);
  if (dpdCol) {
    const overdue = indices.filter((i) => toNumber(redactedRows[i][dpdCol]) > 0);
    if (overdue.length) indices = overdue;
  }
  indices = indices.slice(0, 8);
  const subset = indices.map((i) => redactedRows[i]);

  const prompt = buildDraftPrompt({ headers, rows: subset, instruction });
  ...
  if (!drafts) { drafts = fallbackDrafts({ headers, rows: subset }); usedFallback = true; }
  ...
  return {
    ...
    protectedCount,
    protectedByType,
    headline: `Drafted ${letters.length} bilingual ${/remind|overdue|collect|due/i.test(instruction) ? "payment reminder" : "message"}${letters.length === 1 ? "" : "s"} — English and Arabic.`,
    letters,
